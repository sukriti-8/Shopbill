...
class InvoiceViewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final invoiceNumber = invoice['invoiceNo'];

    final partyName = invoice['party'] ?? '';

    final address = invoice['address'] ?? '';

    final gstNo = invoice['gstNo'] ?? '';

    final taxableValue =
        (invoice['taxableValue'] ?? 0).toDouble();

    final cgstPercent =
        (invoice['cgstPercent'] ?? 0).toDouble();

    final sgstPercent =
        (invoice['sgstPercent'] ?? 0).toDouble();

    final igstPercent =
        (invoice['igstPercent'] ?? 0).toDouble();

    final cgstAmount =
        (invoice['cgstAmount'] ?? 0).toDouble();

    final sgstAmount =
        (invoice['sgstAmount'] ?? 0).toDouble();

    final igstAmount =
        (invoice['igstAmount'] ?? 0).toDouble();
